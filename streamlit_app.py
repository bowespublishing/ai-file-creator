...
# ——— Computed text ———
eta_text = f"Estimated back online: <b>{ETA}</b>" if ETA else "We’ll be back shortly."
eta_dt   = parse_eta_iso(ETA_ISO) if ETA_ISO else None
countdown_line = f" <span style='opacity:.85'>(~{human_delta(eta_dt)})</span>" if eta_dt else ""
contact_html = f'Questions? Email <a href="{email_link(CONTACT_EMAIL)}"><b>{CONTACT_EMAIL}</b></a>.' if CONTACT_EMAIL else ""
...
# ——— Page config ———
st.set_page_config(page_title=f"{BRAND_NAME} — Maintenance", page_icon="🛠️", layout="wide")

# ——— Full HTML (rendered via components.html to avoid Markdown parsing) ———
...
<style>
  /* Remove Streamlit chrome inside iframe body if any */
  body {{ margin: 0; }}
  :root {{
    --accent-a: {ACCENT_A};
    --accent-b: {ACCENT_B};
  ...
  @media (max-width: 680px) {{
    .row {{ grid-template-columns: 1fr; text-align: center; gap: 18px; }}
  }}
  ...
  p  {{ margin: 0; opacity: .92; font-size: clamp(15px, 1.6vw, 18px); }}
...
<div class="wrap">
  <div class="card" role="region" aria-label="Maintenance notice">
    <div class="row">
      <div class="logo">
        {"<img src='" + logo_uri + "' alt='Logo'/>" if logo_uri else f"<div class='logo-fallback' aria-label='Logo'>{initials}</div>"}
      </div>
      <div>
        <h1>{BRAND_NAME}</h1>
        <p>{TAGLINE}</p>
...
# Render via iframe (prevents Markdown from converting HTML to a code block)
