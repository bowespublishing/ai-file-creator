...
st.markdown(
    ...
    <style>
      ...
      /* Card page layout (pure HTML so no empty wrappers) */
      .maint-wrap {{
        min-height: 82vh;
    ...
    <div class="maint-wrap">
      <div class="maint-card">
        <div class="maint-row">
          <div class="logo-wrap">
            {"<img class='logo-img' src='" + logo_uri + "' alt='Logo'/>" if logo_uri else f"<div class='logo-fallback' aria-label='Logo'>{(''.join([w[0] for w in BRAND_NAME.split() if w[:1].isalnum()])[:3] or '✨').upper()}</div>"}
          </div>
          <div class="brand">
            <h1>{BRAND_NAME}</h1>
            <p>{TAGLINE}</p>
